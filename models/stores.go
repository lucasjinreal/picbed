...
func DisableStores(receive common.MenuReceive) bool {
...
func EnableStores(receive common.MenuReceive) bool {
...
func RankStores(receive common.MenuReceive) bool {
	for k, v := range receive.Enable {
		err := db.Model(&Store{}).
...
func setCacheStores(key string, value *[]Store) bool {
	err := config.Cache.Put("store_"+key, value, 3600)
	return modelsError(common.ErrorToString(err))
}

func deleteCacheAllStores() {
	config.Cache.Delete("store_all")
	config.Cache.Delete("store_active")
}
